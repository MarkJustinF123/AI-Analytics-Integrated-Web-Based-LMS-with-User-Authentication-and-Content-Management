  ...
  <div class="row">
    ...
    <main class="col-md-9 col-lg-10 ms-sm-auto px-md-4 main-content">
      ...
      <div class="d-flex justify-content-between align-items-center pb-3 mb-4 border-bottom">
        <h2 class="h2 fw-bold m-0">Edit Quiz</h2>
        <div class="dropdown">
          ...
          <div class="col-lg-8">
              ...
              <div class="form-card">
                  <div class="mb-4">
                      ...
                      <input type="text" class="form-control form-control-lg" id="quiz-title" placeholder="Enter title...">
                  </div>
                  ...
                  <div class="row mb-4">
                      ...
                      <div class="col-md-6">
                          ...
                          <input type="date" class="form-control" id="quiz-due">
                      </div>
                  </div>
                  ...
                  <div class="d-flex justify-content-end">
                      <button class="btn-update" id="post-btn">Update Quiz</button>
                  </div>
              </div>
...
<script>
  ...
  let uploadedFiles = []; // Stores {id, name}

  // --- LOAD DATA ---
  async function loadData() {
    // Robust Fetch for ID vs DocumentID
    let url = `http://localhost:1337/api/quizzes/${quizId}?populate=*`;
    if(isNaN(quizId)) url = `http://localhost:1337/api/quizzes?filters[documentId][$eq]=${quizId}&populate=*`;
    ...
    try {
      ...
      const quiz = Array.isArray(result.data) ? result.data[0] : result.data;
      if (!quiz) throw new Error('Quiz data empty.');

      document.getElementById('quiz-title').value = quiz.title || '';
      document.getElementById('quiz-description').value = quiz.description || '';
      document.getElementById('quiz-points').value = quiz.points || '100';
      document.getElementById('quiz-link').value = quiz.link || '';
      if(quiz.dueDate) document.getElementById('quiz-due').value = quiz.dueDate;

      // Load Files
      let files = [];
      if (quiz.files && Array.isArray(quiz.files)) files = quiz.files; 
      else if (quiz.files && quiz.files.data) files = quiz.files.data;
  ...
  document.getElementById('post-btn').addEventListener('click', async () => {
    ...
    let url = `http://localhost:1337/api/quizzes/${quizId}`;
    const body = { data: { title, description: desc, points, dueDate: due || null, link, files: uploadedFiles.map(f => f.id) } };
    ...
    } catch (error) { console.error(error); alert('Error updating.'); btn.disabled = false; btn.textContent = 'Update Quiz'; }
  });
